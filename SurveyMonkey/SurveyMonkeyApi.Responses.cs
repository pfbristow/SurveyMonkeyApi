...
public partial class SurveyMonkeyApi
{
    ...
    private Response GetResponseRequest(long objectId, ObjectType source, long responseId, bool details)
    {
        ...
        var endPoint = $"/{source.ToString().ToLower()}s/{objectId}/responses/{responseId}{detailString}";
        var requestData = details ? RequestSettingsHelper.GetPopulatedProperties(new GetResponseListSettings{Simple = true}) : [];
        var result = MakeApiGetRequest(endPoint, requestData);
    ...
    private async Task<Response> GetResponseRequestAsync(long objectId, ObjectType source, long responseId, bool details)
    {
        ...
        var endPoint = $"/{source.ToString().ToLower()}s/{objectId}/responses/{responseId}{detailString}";
        var requestData = details ? RequestSettingsHelper.GetPopulatedProperties(new GetResponseListSettings { Simple = true }) : [];
        var result = await MakeApiGetRequestAsync(endPoint, requestData);
        var responses = result.ToObject<Response>();
        return responses;
    }
    ...
    private List<Response> GetResponseListPager(long id, ObjectType objectType, GetResponseListSettings settings, bool details)
    {
        ...
        var endPoint = $"/{objectType.ToString().ToLower()}s/{id}/responses{bulk}";
        var maxResultsPerPage = details ? 100 : 1000;
    ...
    public async Task DeleteCollectorResponsesAsync(long collectorId)
    {
        var endPoint = $"/collectors/{collectorId}/responses";
